...
execute as @a[team=RED,scores={death_detector=1..},nbt={Health:20.0f}] run function warfare:one_hill/death_red
execute as @a[team=BLUE,scores={death_detector=1..},nbt={Health:20.0f}] run function warfare:one_hill/death_blue
...
execute if score COUNTDOWN_CLOCK clock matches 1 run function warfare:one_hill/start
...
execute if score GAME_CLOCK clock matches 1 run function warfare:one_hill/game_over
...
# execute as @a[scores={clock=1},team=RED] run function warfare:one_hill/respawn_red
# execute as @a[scores={clock=1},team=BLUE] run function warfare:one_hill/respawn_blue


# respawn
execute as @a[scores={clock=1}] run scoreboard players set @s player_respawning 1
execute as @a[team=RED,scores={player_respawning=1}] as @e[type=armor_stand,name="respawn_red",limit=1,sort=random] at @s unless entity @a[team=!RED,distance=..5] run function warfare:one_hill/respawn_red
execute as @a[team=BLUE,scores={player_respawning=1}] as @e[type=armor_stand,name="respawn_blue",limit=1,sort=random] at @s unless entity @a[team=!BLUE,distance=..5] run function warfare:one_hill/respawn_blue
...
execute if score second_detector clock matches 2 run function warfare:one_hill/second
scoreboard players operation second_detector clock = GAME_CLOCK clock
...
# tick hill particles (1/5 speed)
scoreboard players operation second_detector clock = GAME_CLOCK clock
scoreboard players operation second_detector clock %= CONST_10 utility
execute if score second_detector clock matches 2 run function warfare:one_hill/hill/border_particles

execute as @e[type=armor_stand,name="hill"] at @e[type=armor_stand,name="hill"] as @e[distance=..10,team=RED] run particle dust_color_transition 1 1 1 1 1 0 0 ~ ~1 ~ 3 2 3 1 8 normal
execute as @e[type=armor_stand,name="hill"] at @e[type=armor_stand,name="hill"] as @e[distance=..10,team=BLUE] run particle dust_color_transition 1 1 1 1 0 0 1 ~ ~1 ~ 3 2 3 1 8 normal
