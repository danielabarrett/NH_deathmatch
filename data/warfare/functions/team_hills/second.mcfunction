...
execute if score GAME_CLOCK clock >= CONST_60 utility if score minute_detector clock matches 30 run tellraw @a {"text":"The Hill moves in 30 seconds!","color":"gold"}
execute if score GAME_CLOCK clock >= CONST_60 utility if score minute_detector clock matches 10 run tellraw @a {"text":"The Hill moves in 10 seconds!","color":"red"}
