...
execute if entity @p[team=BLUE,scores={kill_detector=1..}] run scoreboard players add BLUE killcount 1
# other death
execute unless entity @p[team=BLUE,scores={kill_detector=1..}] run tellraw @a [{"selector":"@s"},{"text":" died"}]
execute unless entity @p[team=BLUE,scores={kill_detector=1..}] run scoreboard players remove RED killcount 1
